...
- **Download** the latest Windows ZIP package from the [MONICA Releases Page](https://github.com/zalf-rpm/monica/releases), for example: **`monica_win64_<version>.zip`**
...
Because the ZIP version isn’t registered automatically on Windows, users can either:
...
1. Add the path to bin folder (e.g. `C:\monica\bin`) to the PATH environment variable

2. Create a new environment variable named `MONICA_PARAMETERS` and set its value to: `C:\monica\monica-parameters`
...
4. Change to a project directory containing `sim.json`, `crop.json`, `site.json`, and `climate.csv`
```
cd C:\monica\projects\Hohenfinow2-single-run
```

Then run `monica-run.exe -o .\sim-out.csv sim.json`. The sim JSON file must be supplied when running MONICA manually.

---

## 3. Updating an Existing Installation

- If the user already has an older MONICA installation for example, under **`%USERPROFILE%\AppData\MONICA`**, users can simply replace the files in the existing `bin` folder with those from the new ZIP.
