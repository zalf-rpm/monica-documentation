...
To simplify cluster access and standardize simulation workflows, the [Model and Simulation Infrastructure](https://www.zalf.de/en/struktur/cdp/mas/Pages/default.aspx) working group provides a web-based interface called [**Rundeck**](https://www.rundeck.com/).
...
2. Log in using your HPC credentials:
    ...
    - Username  
    - Password + Yubikey (2-factor authentication)
...
- **git**: clone a repository for this run
    - If selected, provide the repository URL. Example: `https://github.com/zalf-rpm/monica_example.git`
- **folder**: use an existing folder on the cluster
    - If selected, provide an absolute path on BeeGFS
...
- The job will be stopped when this time limit is reached, so make sure to choose a value that is long enough for your simulation to complete.
...
- Use this field to pass extra command-line arguments to the producer and consumer scripts.
- Parameters are passed to the scripts after the standard Rundeck arguments.
- You can write them as key-value pairs or in any format that your Python scripts are able to parse.
- Make sure that your producer script is written to read these additional parameters.

**Consumer Memory**

Controls the memory allocated to the consumer script. The consumer usually needs more memory if it stores many simulation results in memory before writing them to files.
...
Use `tiny` if the consumer writes results directly to files. Use a larger option if the consumer aggregates large outputs, stores many grid cells, or keeps long time series in memory.

**Producer Memory**

Controls the memory allocated to the producer script.
...
Use `tiny` for simple setup generation. Use `normal` or higher if the producer loads large spatial datasets or performs preprocessing before sending simulations.

**Proxy Memory**

Controls the memory allocated to the MONICA proxy process.
...
For most jobs, `tiny` or `normal` is sufficient. Use `high` or `veryhigh` if the job produces large outputs or uses many workers.
