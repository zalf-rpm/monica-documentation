...
| Parameter name                                                                          | Unit/Type             | Default  | Example                                              | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
|-----------------------------------------------------------------------------------------|-----------------------|----------|------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **`exported`**                                                                          | **true** or **false** | **true** | `"exported": false`                                  | Controls the default harvest behavior when no organ-specific harvest settings are defined. If omitted, `exported` defaults to **`true`**. When **`exported = true`**, the organs defined as yield by the cultivar parameters **`OrganIdsForPrimaryYield`** and **`OrganIdsForSecondaryYield`** are removed from the field. The remaining aboveground biomass is returned to the soil together with the remaining living root biomass. When **`exported = false`**, the remaining living root biomass and all aboveground biomass are returned to the soil. If any organ-specific settings (`leaf`, `fruit`, `shoot`, `struct`, `sugar`) are provided, `exported` is ignored and harvest is controlled by those organ-specific settings. |
| **`leaf`** and/or **`fruit`** and/or **`shoot`** and/or **`struct`** and/or **`sugar`** | JSON object           |          | `"leaf": {"export": [85, "%"], "incorporate": true}` | Defines organ-specific harvest behavior. `export` specifies the percentage of the selected organ biomass that is removed from the field (**default: 100%**). The non-exported part of that organ remains in the field. If `incorporate = true`, it is incorporated into the soil. If `incorporate = false`, it remains on the soil surface as overlay biomass. Any organ not explicitly listed is not exported and is incorporated into the soil by default, equivalent to `{"export": 0, "incorporate": true}`. When at least one organ-specific setting is provided, the global `exported` parameter is ignored.                                                                                                                      |
